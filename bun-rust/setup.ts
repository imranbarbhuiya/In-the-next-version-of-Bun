...
function extractDelimited(
  ...
  const endIdx = text.indexOf(
    endMarker,
    afterStart === -1 ? startIdx : afterStart + 1
  );
  if (endIdx === -1) return null;
  const contentStart =
    afterStart === -1 ? startIdx + startMarker.length : afterStart + 1;
  ...
  try {
    ...
    if (isCargoMode) {
      ...
      const hashKey = sha256(
        JSON.stringify({ mode: "cargo", depsStr, sourceCode })
      );
